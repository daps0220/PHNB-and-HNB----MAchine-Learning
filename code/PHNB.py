...
def all_probability_dict(key_pair,key_aj_c_pair,key_ai_c_pair,X,y,prior_class_prob):
    ...
    #prob_ai_given_aj_c = {k: round((float(v)+(1.0/X.shape[1]))/((find_prob_ai_c(X,y,float(k.split("#")[1]),int(k.split("#")[2]),int(k.split("#")[3]),prior_class_prob[int(k.split("#")[2])]))[0]+1.0),5) for k,v in prob_ai_aj_c.items()}
    #prob_ai_given_aj_c = {k: round((float(v)+(1.0/cols))/(prob_aj_c[k.split("#")[1]+"#"+k.split("#")[2]+"#"+k.split("#")[4]]+1.0),5) for k,v in prob_ai_aj_c.items()}
...
def package_attribute(IP_matrix):

    no_of_attr = IP_matrix.shape[0]


    HNB_bag = []
    NB_bag = []
    thresold = float(2 * np.sum(IP_matrix)) / (no_of_attr *(no_of_attr - 1))

    for r in range(IP_matrix.shape[0]):

        #print (IP_matrix[r]thresold).sum()
        if ((IP_matrix[r]>thresold).sum()==(IP_matrix.shape[0]-1)):
            HNB_bag.append(r)
        else:
            NB_bag.append(r)
    ...
    return HNB_bag,NB_bag

def prob_ai_c_NB(X_test_example,cl,prob_ai_c,NB_bag,y_train,X_train,prob_c):

    product_prob = 1
    for index in NB_bag:
        key = str(X_test_example[index])+"#"+str(cl)+"#"+str(index)

        prob1 = find_prob_ai_c(X_train,y_train,X_test_example[index],cl,index,prob_c)[1]
        if(prob1!=0):
            product_prob*= prob1
        else:
            product_prob*=1
...
def membership_function(X_test,Wij_matrix,HNB_bag,NB_bag,prob_ai_given_aj_c_each,cl,prob_ai_c,prob_class,y_train,X_train):
    ...
    for example in range(X_test.shape[0]):
        if(len(HNB_bag)!=0):
            prob_HNB=prob_ai_hiddenai_c(X_test[example],Wij_matrix,prob_ai_given_aj_c_each,cl,HNB_bag)
        if(len(NB_bag)!=0):
            prod_NB = prob_ai_c_NB(X_test[example],cl,prob_ai_c,NB_bag,y_train,X_train,prob_class)

        product_prob_hidden[example] = prob_class*prob_HNB*prod_NB
...
def prob_ai_hiddenai_c(X_test_each,W_matrix,prob_ai_given_aj_c,cl,HNB_bag):
    ...
    """for example in range(X_test.shape[0]):
        ...
        product_prob_hidden = product_prob_hidden * prior_prob_c

    #print product_prob_hidden
    #exit(0)
    return product_prob_hidden"""
    prob_each_attribute = np.zeros(len(HNB_bag),dtype=np.float)
    for ai in HNB_bag:
        hidden_prob_sum=0
        for aj in range(X_test_each.shape[1]):
            if(aj!=ai):
                key = str(X_test_each[ai])+"#"+str(X_test_each[aj])+"#"+str(int(cl))+"#"+str(ai)+"#"+str(aj)
                if(key in prob_ai_given_aj_c):
                    hidden_prob_sum+= W_matrix[ai][aj] * prob_ai_given_aj_c[key]
                else:
                    hidden_prob_sum+=0
    prob_each_attribute[HNB_bag.index(ai)] = hidden_prob_sum
    return np.prod(prob_each_attribute)
...
def classify_X(X,y,fold):
    ...
    for train_index, test_index in kf:
       ...
       HNB_bag,NB_bag = package_attribute(IP_matrix)
       ...
       for cl in np.unique(y):
           member_fun = membership_function(X_test,Wij_matrix,HNB_bag,NB_bag,prob_ai_given_aj_c_each,cl,prob_ai_given_c,prior_class_probability[cl],y_train,X_train)
           #member_fun = prob_ai_hiddenai_c(X_test,Wij_matrix,prob_ai_given_aj_c_each,cl,prior_class_probability[cl])
...
if __name__ == '__main__':
    ...
    #print single_var
    #X_val = np.array(single_var[:,2:],dtype='float64')
